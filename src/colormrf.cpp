...
void MyFrame::OnPaint(wxPaintEvent& event)
{
  ...
  pDC.DrawText(str, 535, 560);
  str.Printf("global energy = ");
  pDC.DrawText(str, 535, 595);
  str.Printf("T = ");
  pDC.DrawText(str, 535, 630);
  str.Printf("CPU time = ");
  pDC.DrawText(str, 535, 665);
  str.Printf("Height");
  pDC.DrawText(str, 535, 700);
  str.Printf("Width");
  pDC.DrawText(str, 535, 735);
  pDC.DrawText(wxString() << imageop->GetK(), 645, 560);
  pDC.DrawText(wxString() << imageop->GetE(), 645, 595);
  pDC.DrawText(wxString() << imageop->GetT(), 645, 630);
  pDC.DrawText(wxString() << imageop->GetTimer() << " ms", 645, 665);
  event.Skip();	
}
  ...
  : wxFrame((wxFrame *)NULL, -1, title, pos, size)
{
  ...
  save_button = new wxButton(this, ID_SAVE_BUTTON, "Save", wxPoint(595,480));
  doit_button = new wxButton(this, ID_DOIT_BUTTON, "Do it >>", 
			     wxPoint(558,150));
  doit_button->Disable();
  select_region_button = new wxButton(this, ID_SELECTREGION_BUTTON, 
				      "Select classes", wxPoint(218,521));
  select_region_button->Disable();
  wxString choices[5] = {"Metropolis", "Gibbs sampler", "ICM", "MMD", "BeesAlg"};
  ...
  //  tbeta->SetValue("2.5");
  *tbeta << 2.5;
  tt = new wxTextCtrl(this, ID_T, "", wxPoint(212,556), 
  ...
  *tt << 0.05;
  ...
  // tT0->SetValue("4.0");
  *tT0 << 4.0;
  tc = new wxTextCtrl(this, ID_C, "", wxPoint(212,591), wxSize(60,20), 
		      wxTE_PROCESS_ENTER|wxTE_RIGHT, 
		      *(new wxTextValidator(wxFILTER_NUMERIC)));
  tc->SetMaxLength(8);
  // tc->SetValue("0.98");
...
void ImageOperations::placeRandomlyScouts(int n, int start) {
	...
	while (count < n) {
		...
		flower.x = irh;
		flower.y = irw;
		bool exists = scoutExists(flower, count);
		if (!exists) {
			flower.pollen = 0;
